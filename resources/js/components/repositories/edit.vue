								...
								<div class="card">
									<div class="card-header pt-5">
										<div class="card-title d-flex flex-column">
											...
											<!--begin::Title-->
											<span class="pt-1 fw-semibold fs-6">Update a new category</span>
											<!--end::Title-->
											</div>
											<!--end::Info-->
										</div>
									...
									<div class="card-body">
										<form @submit.prevent="update">
											...
											<div class="mt-5">
												<button type="submit" class="btn btn-primary" v-if="isSaving === false">Update</button>
...
onMounted(() => {
	...
	store.dispatch('user/fetchSingleCategory', id);
})

const singleCategoryItem = reactive({
	all: '',
})

singleCategoryItem.all = computed(() => store.getters['user/getSingleCategory'])
console.log(singleCategoryItem.all)

name = singleCategoryItem.all.name
type = singleCategoryItem.all.type
...
const update = async ()=> {
        Csrf.getCookie();
        isSaving.value = true;
        http.post(`${baseURL}/update-categories/${id}`, {
            'name' : name,
			// 'type' : type,
            '_method' : 'PATCH',
        })
